...
find_met_files <- function(t_start, n_hours, met_path,
  ...
  # Find the files that match the request by per-day name
  ...
  # Dedupe by physical file (inode) so multi-day hardlink/symlink aliases of
  # one ARL archive are listed exactly once in the HYSPLIT CONTROL file.
  matched <- unique(available[idx])
  info <- normalizePath(matched, mustWork = FALSE)
  matched[!duplicated(info)]  # Available files that match the request
}
