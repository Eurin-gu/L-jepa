#!/usr/bin/env bash
set -x
cat > /root/master_all.py << "PYEOF"
import cdsapi, os, datetime, subprocess, io, time
...
def fetch_combined(tag, area, days):
    ...
    for suffix, kind, dsname, var, lev in [
        ...
        outs.append(out)
    return outs
def split_convert(tag, region, days):
    import eccodes
    mdir = "/root/met_era5/" + region
    os.makedirs(mdir, exist_ok=True)
    for combined, suffix in zip(["%s/%s_c_p.grib"%(GRIB,tag), "%s/%s_c_s.grib"%(GRIB,tag)], ["_p","_s"]):
        ...
        for k,(p,b) in bufs.items():
            data = b.getvalue()
            if len(data) > 300000 and (not os.path.exists(p) or os.path.getsize(p) < 300000):
    ...
    return okall
def run_batch(region, date, mdir):
    ...
    print("batch", region, date, "rc", r.returncode, tail.replace(chr(10)," | ")[-300:], flush=True)
for tag,(region, area, dates) in REGIONS.items():
    for ds in dates:
        ...
        if both and os.path.exists(man):
            print("skip done", region, ds, flush=True); continue
        print("PROCESS", region, ds, flush=True)
        fetch_combined(tag, area, days)
        if split_convert(tag, region, days):
            run_batch(region, ds, mdir)
        print("DONE", region, ds, flush=True)
print("MASTER ALL DONE", flush=True)
PYEOF
nohup /root/venvs/cds/bin/python /root/master_all.py > /root/master_all.log 2>&1 &
echo started pid $!